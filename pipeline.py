...
def pipeline(args):
    ...
    for key in input_nodes:
        shapes[key] = len([item for item in feature_name if item.startswith(key) and 'attribute' not in item])
        tmp = len([item for item in feature_name if item.startswith(key) and 'attribute' in item])
        if tmp > 0:
            shapes['Attribute_'+key] = tmp
    # Build Graph
    ...
    optimizer = adam = optimizer.apply_gradients(capped_gvs)
