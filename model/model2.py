...
def Build(input_size):
    ...
    for key in keys:
        for item in task1_output[key]:
            ...
            labels[label_name] = tf.cond(is_training, 
                           ...
                           tf.random_normal(tf.shape(labels[label_name]), mean=0, stddev=0.003) * labels[label_name] + \
                           tf.random_normal(tf.shape(labels[label_name]), mean=0, stddev=0.01),
                           lambda : labels[label_name])
    ...
    for node in link:
        ...
        # LSTM Cell
        # encoder_single_cell_fw = tf.contrib.rnn.BasicLSTMCell(rnn_dim_hidden, state_is_tuple=True)#BNLSTMCell(rnn_dim_hidden, True) #tf.contrib.rnn.GRUCell(rnn_dim_hidden) #tf.contrib.rnn.BasicLSTMCell(rnn_dim_hidden, state_is_tuple=True) #tf.contrib.rnn.GRUCell(rnn_dim_hidden)
        # decoder_single_cell_fw = tf.contrib.rnn.BasicLSTMCell(rnn_dim_hidden, state_is_tuple=True) #tf.contrib.rnn.GRUCell(rnn_dim_hidden) #tf.contrib.rnn.BasicLSTMCell(rnn_dim_hidden, state_is_tuple=True) #tf.contrib.rnn.GRUCell(rnn_dim_hidden)
        
        encoder_nodes_fw[node] = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.GRUCell(4), tf.contrib.rnn.GRUCell(2), tf.contrib.rnn.GRUCell(2)])
        decoder_nodes_fw[node] = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.GRUCell(4), tf.contrib.rnn.GRUCell(2), tf.contrib.rnn.GRUCell(2)]) 
        ...
        # output_fw[node] = encoder_nodes_fw[node].zero_state(tf.shape(inputs[node])[0], tf.float32)

    # Build Graph
    # with tf.variable_scope("Embedding", regularizer=tf.contrib.layers.l1_regularizer(0.001)):
    #     # Embedding of node
    #     for node in link:
    #         embeddings[node] = tf.Variable(tf.random_uniform([embedding_num, embedding_feature_num], -0.1, 0.1), 
    #                                                           name= node + '_embedding', 
    #                                                           trainable=True)
    #     # for key in labels:
    #     #     embedding_output[key] = tf.Variable(tf.random_uniform([embedding_num, 1], -0.2, 0.2), 
    #     #                                                       name= key + '_embedding', 
    #     #                                                       trainable=True)
    ...
    with tf.variable_scope("RNN", 
                            initializer=tf.contrib.layers.variance_scaling_initializer(factor=0.3, 
        ...
        for timestep in range(encoder_num_timesteps):
            ...
            for window in range(window_num):
                with tf.variable_scope('GC') as scope:
                ...
                for node in link:
                    ...
                    # input_data = tf.cond(is_training, 
                    #        lambda : tf.nn.dropout(input_data, keep_prob=0.95),
                    #        lambda : input_data)
                    ...
                    data = tf.concat(axis=1, values=[input_data, gc[node]])  
                    # RNN
                    with tf.variable_scope('Encoder-GRU-{}'.format(cfg.model.node_type[node]),
                                            initializer=tf.orthogonal_initializer(gain=0.3)) as scope: 
        ...
        task1_prediction = {}
        task2_prediction = {}
        with tf.variable_scope("task1_net", regularizer=tf.contrib.layers.l2_regularizer(0.03)):
                ...
                for i in range(2):
                    ...
                    with tf.variable_scope("{}_{}".format(node, target)):
                        for timestep in range(decoder_num_timesteps):
                            ...
                            # data = tf.cond(is_training, 
                            #                lambda : tf.nn.dropout(data, keep_prob=0.95),
                            #                lambda : data)
                            
                            if timestep > 0:
                                tf.get_variable_scope().reuse_variables()
                            data = FC(x=data, in_dim=data.get_shape()[1].value, out_dim = 1, name='{}_{}_{}'.format(node, target, 'fc1'), with_bn=False)
                            # embedding  = tf.nn.embedding_lookup(embedding_output['{}_{}'.format(node, target)], time_now)
                            # data = data + embedding
                ...
                for item in range(num_output):
                    ...
                    with tf.variable_scope('{}_{}'.format(key, item)):
                        ...
                        for timestep in range(decoder_num_timesteps):
                            ...
                            # embedding  = tf.nn.embedding_lookup(embedding_output['{}_{}'.format(key, item)], time_now)
                            # data = data + embedding
                            pred_result.append(data)
