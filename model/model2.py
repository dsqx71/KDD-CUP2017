...
def Build(input_size):
    ...
    weekday = tf.placeholder(tf.int32, shape=(None), name = 'weekday')
    ...
    weather = tf.cond(is_training,
                       lambda : weather+ \
                       tf.random_normal(tf.shape(weather), mean=0, stddev=0.01) * weather + \
                       tf.random_normal(tf.shape(weather), mean=0, stddev=0.003),
                       lambda : weather)
    attribute = {}
    ...
    with tf.variable_scope("Weather_attribute_Embedding",
        ...
        with tf.variable_scope("Attribute"):
            ...
            for node in attribute:
                if Flag is  True:
                    ...
                    Flag = True
                # Level0
                tmp = tf.cast(tf.expand_dims(time, 1), tf.float32)
                tmp.set_shape([None, 1])
                data = tf.concat([attribute[node], tmp], axis=1)
                data = FC(x=data, in_dim=data.get_shape()[1], out_dim=data.get_shape()[1], name='FC0.0', activation='prelu', is_training=True, with_bn=False)
        ...
        with tf.variable_scope("Embedding"):
            ...
            for node in link:
                ...
                #                                                   trainable=True)
                embeddings_weekday[node] = tf.Variable(tf.random_uniform([7, embedding_feature_num], -1.0, -1.0),
                                                                  name= node + '_Embedding_weekday',
                                                                  trainable=True)
    ...
    for node in link:
        ...
        data_tmp = tf.cond(is_training,
                           lambda : data_tmp + \
                           tf.random_normal(tf.shape(data_tmp), mean=0, stddev=0.01) * data_tmp + \
                           tf.random_normal(tf.shape(data_tmp), mean=0, stddev=0.003),
                           lambda : data_tmp)

        with tf.variable_scope('Extracting_feature_{}'.format(cfg.model.node_type[node]),
                                initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                regularizer=tf.contrib.layers.sum_regularizer([tf.contrib.layers.l2_regularizer(1E-3),
                                                                               tf.contrib.layers.l1_regularizer(1E-3)])) as scope:
            ...
            data = data_tmp
            data = conv(data=data, name='conv1', width=1, in_dim=data.get_shape()[2], out_dim=data.get_shape()[2]//2, stride=1, pad='SAME')
    ...
    with tf.variable_scope("Recurrent-Network",
                           initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                           regularizer=tf.contrib.layers.sum_regularizer([tf.contrib.layers.l2_regularizer(1E-4),
                                                                          tf.contrib.layers.l1_regularizer(1E-4)])) as scope:
        ...
        for timestep in range(encoder_num_timesteps):
            ...
            for window in range(window_num):
                ...
                for node in link:
                    ...
                    weekday_embedding = tf.nn.embedding_lookup(embeddings_weekday[node], weekday)
                    weekday_embedding.set_shape([None, embedding_feature_num])
                    # if node in attribute:
                    #     embedding = embedding + attribute[node]
                    # data = tf.concat(axis=1, values=[input_data, embedding, weather_now)
                    if node in attribute:
                        data = tf.concat(axis=1, values=[input_data, attribute[node], weekday_embedding ,weather_now, gc[node]])
                    else:
                        data = tf.concat(axis=1, values=[input_data, weekday_embedding ,weather_now, gc[node]])
                    # RNN
        ...
        for timestep in range(encoder_num_timesteps, encoder_num_timesteps+decoder_num_timesteps):
            ...
            for window in range(window_num):
                ...
                for node in link:
                    ...
                    # if node in attribute:
                    #     embedding = embedding + attribute[node]
                    # data = tf.concat(axis=1, values=[embedding, weekday_embedding, weather_now, gc[node], attention])
                    if node in attribute:
                        data = tf.concat(axis=1, values=[attribute[node], weekday_embedding, weather_now, gc[node], attention])
                    else:
                        data = tf.concat(axis=1, values=[weekday_embedding, weather_now, gc[node], attention])
                    # RNN
        ...
        regularizer=tf.contrib.layers.sum_regularizer([tf.contrib.layers.l2_regularizer(1E-2),
                                                       tf.contrib.layers.l1_regularizer(1E-2)]),
        initializer=tf.contrib.layers.xavier_initializer(uniform=False),):
